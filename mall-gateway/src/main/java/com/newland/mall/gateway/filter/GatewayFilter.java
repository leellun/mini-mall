package com.newland.mall.gateway.filter;

import com.alibaba.fastjson.JSONObject;
import com.alibaba.fastjson2.JSON;
import com.newland.mall.constant.AuthConstant;
        ...
        if (!StringUtils.hasText(token)) {
            ...
            try {
                ...
                if (loginUser != null) {
                    ...
                    org.springframework.http.server.reactive.ServerHttpRequest tokenRequest = exchange.getRequest().mutate().header("json-token", "").build();
                    ServerWebExchange build = exchange.mutate().request(tokenRequest).build();
                    return chain.filter(build);
                }
            } catch (Exception e) {
